...
declare const DOCUMENTATION_OPTIONS: {
  FILE_SUFFIX: string
  LINK_SUFFIX: string
  HAS_SOURCE: boolean
}
...
interface SphinxSearchResult {
  ...
  title: string
  anchor: string
  objectDescr: string | null
  score: number
  filename: string
}

// Use `config.base` rather than `DOCUMENTATION_OPTIONS.URL_ROOT` to
// resolve all root-relativepaths in order to
// suport instant navigation.
...
/**
 * Returns a full URL for a given document.
 *
 * @param path - Document id.
 * @returns The full document URL.
 */
function getDocumentUrl(path: string): string {
  return getAbsoluteUrl(path + DOCUMENTATION_OPTIONS.LINK_SUFFIX)
}
...
interface SearchIndex {
  docnames: string[]
  filenames: string[]
  objects: Record<string, Record<string, [documentIndex: number, objectTypeIndex: number, priority: number, anchor: string]>>
...
function performObjectSearch(objectTerm: string, otherterms: string[]): SphinxSearchResult[] {
  const {filenames, docnames, objects, objnames, titles} = searchIndex
...
function performTermsSearch(
  ...
  const {docnames, filenames, titles, terms, titleterms} = searchIndex
...
async function convertSphinxResult(
  ...
  const location = getDocumentUrl(result.docname) + result.anchor
  ...
  if (result.objectDescr) {
    // For object matches, we don't attempt to find a snippet.
    return [
      {
        location,
        score: result.score,
  ...
  const requestUrl = getDocumentUrl(result.docname)
